...
const FacultyDashboard = () => {
  ...
  const showFeedbackPopup = (submissionId) => {
    document.getElementById(`feedback-popup-${submissionId}`).style.display =
      "block";
    document.getElementById(`feedback-overlay-${submissionId}`).style.display =
      "block";
  };
  ...
  const exportSubmissionsToExcel = () => {
    ...
    try {
      ...
      const submissionsData = selectedAssignment.submissions.map(
        (submission, index) => {
          // Get student info
          const student = submission.student ||
            selectedClassroom.students?.find(
              (s) => s._id === submission.studentId
          ...
          return {
            ...
            Grade:
              submission.grade !== undefined
                ...
                : "N/A",
            "Grading Method": submission.isAutoGraded
              ? "AI Graded"
              : submission.grade !== undefined
              ? "Manually Graded"
              : "Not Graded",
      ...
      // Create filename
      const filename = `${selectedAssignment.title}_submissions_${
      ...
      const validateUrls = async () => {
        ...
        for (const submission of selectedAssignment.submissions) {
          if (submission.submissionUrl) {
            validationResults[submission._id] = await checkUrlExists(
  ...
  const handleCreateClassroom = async (formData) => {
    try {
      ...
      const studentEmails =
        typeof formData.studentEmails === "string"
      ...
      try {
        ...
        if (response.data.success) {
          ...
          setSelectedClassroom((prevClassroom) => ({
            ...prevClassroom,
            students: prevClassroom.students.filter(
    ...
    try {
      ...
      if (testStudent) {
        ...
        assignments.forEach((assignment) => {
          const matchingSubmissions = assignment.submissions?.filter((sub) => {
  ...
  const handleGradeSubmission = async (gradeData) => {
    try {
      ...
      const response = await axios.put(
        getApiUrl(
          `/faculty/assignments/${selectedAssignment._id}/submissions/${selectedSubmission._id}/grade`
        ),
        {
  ...
  const handleEditClick = (classroom, e) => {
    ...
    setFormData({
      ...
      studentEmails:
        classroom.students?.map((student) => student.email).join(", ") || "",
    });
  ...
  const handleReviewAutoGrade = async (
    assignmentId,
    submissionId,
    currentGrade
  ) => {
    try {
      ...
      const response = await axios.put(
        getApiUrl(
          `/faculty/assignments/${assignmentId}/submissions/${submissionId}/grade`
        ),
        {
          grade: parseInt(currentGrade),
          feedback:
            gradeModalData?.feedback || "Reviewed auto-graded submission",
          isReviewed: true,
        },
        {
          headers: {
  ...
  const [processingType, setProcessingType] = useState("single"); // 'single' or 'batch'

  const handleAutoGrade = async (assignmentId, submissionId) => {
    try {
      setProcessingType("single");
      setIsProcessing(true); // Set processing state to true before processing
      const sessionId = sessionStorage.getItem("sessionId");
      const token = sessionStorage.getItem(`token_${sessionId}`);

      setAutoGradingStatus((prev) => ({
        ...prev,
      ...
      const response = await axios.post(
        getApiUrl(
          `/faculty/assignments/${assignmentId}/submissions/${submissionId}/autograde`
        ),
        {},
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        // Only trigger update for the assignment list refresh, not full data reload
        triggerUpdate();
      }
    } catch (error) {
      setError(
        error.response?.data?.message || "Failed to auto-grade assignment"
      );
    } finally {
  ...
  const handleBatchAutoGrade = async () => {
    ...
    // Use processing overlay instead of full page loading
    ...
    for (const submission of ungradedSubmissions) {
      try {
        ...
        const response = await axios.post(
          getApiUrl(
            `/faculty/assignments/${selectedAssignment._id}/submissions/${submission._id}/autograde`
          ),
          {},
          {
            headers: {
        ...
        if (response.data.success) {
          ...
          if (
            completedCount % 3 === 0 ||
            completedCount === ungradedSubmissions.length
          ) {
            setSuccess(
              `Progress: ${completedCount}/${ungradedSubmissions.length} submissions graded`
            );
          }
    ...
    // All done - use processing overlay state instead of full page loading
    setIsProcessing(false);
    setSuccess(
      `Successfully graded ${completedCount} out of ${ungradedSubmissions.length} submissions!`
    );
  ...
  return (
    <div className="relative">
      {isWebsiteLoading && (
        <div
          className="website-loading-overlay"
          style={{
        ...
        >
          <div
            ...
            style={{ background: "rgba(26, 31, 44, 0.8)" }}
          >
            <div className="loading-spinner">
              <i className="fas fa-graduation-cap"></i>
            </div>
            <h1 className="loading-text" style={{ color: "rgb(99, 102, 241)" }}>
          ...
          <div className="processing-content">
            ...
            <h4>
              {processingType === "batch"
                ? "AI is grading all submissions..."
                : "AI is evaluating..."}
            </h4>
            <p className="text-muted">
              {processingType === "batch"
                ? "Please wait while we process all submissions. This may take a few minutes."
                : "Please wait while we process the submission"}
            </p>
          </div>
        </div>
      )}

      <div className="min-vh-100 bg-indigo">
        ...
        >
            ...
            >
              ...
              <div
                style={{
                  ...
                  opacity: 0.1,
                  backgroundImage:
                    "radial-gradient(circle, #ffffff 10%, transparent 10.5%), radial-gradient(circle, #ffffff 10%, transparent 10.5%)",
          ...
          {!selectedClassroom ? (
              ...
              {classrooms.map((classroom, index) => (
                ...
                >
                  ...
                  >
                    ...
                    {/* Description */}
                    <div
                      className="classroom-description mb-3"
                      style={{
                        height: "44px",
                        overflow: "hidden",
                        position: "relative",
                    ...
                    <div className="mt-auto">
                      <div className="row g-2 row-cols-2">
                        ...
                        <div className="col">
                          <div
                            className="p-2 rounded-3 metric-tile"
                  ...
                  <div
                    className="card-footer p-3 border-0"
                    style={{
            ...
            // Assignments View
            <div className="card shadow-sm" style={{ background: "#edf2f7" }}>
                ...
                <div className="classroom-header">
                  <div className="header-main">
                    ...
                    <div className="toggle-buttons">
                      ...
                      <button
                        className={`btn ${
                          activeView === "students" ? "btn-color" : "btn-light"
                        }`}
                    ...
                    <div className="card border-0 shadow-sm">
                      <div className="card-header section-header">
                        ...
                        <div className="section-actions">
                          <button
                            className="btn btn-color btn-sm d-flex align-items-center gap-2"
                      ...
                      <div className="card-body p-0">
                        {assignments.length > 0 ? (
                            ...
                            <table className="table table-hover assignment-table mb-0">
                              ...
                              <tbody>
                                {assignments.map((assignment) => {
                                  ...
                                  const assignmentFirstLetter = assignment.title
                                    .charAt(0)
                                    .toUpperCase();
                                  const assignmentColor = getAssignmentColor(
                                    assignmentFirstLetter
                                  );
                                  ...
                                  const timeClass = getTimeRemainingClass(
                                    assignment.dueDate
                                  );
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      >
                                        <div
                                          className={`d-flex flex-column ${timeClass}`}
                                        >
                                          <span
                                            style={{
                                              fontSize: "0.875rem",
                          ...
                          <div className="text-center py-5">
                            ...
                            <h5 style={{ color: "#4b5563", fontWeight: "500" }}>
                              No assignments yet
                            </h5>
                            <p
                            ...
                            >
                              Create your first assignment for this classroom to
                              get started.
                            </p>
                            <button
                        ...
                        <div className="submission-header">
                          ...
                          <div className="submission-header-bottom">
                            ...
                            <div className="submission-actions">
                              ...
                              {selectedAssignment.submissions?.some(
                                (sub) =>
                      ...
                      <div className="card-body p-0">
                        ...
                        selectedAssignment.submissions.length > 0 ? (
                                  ...
                                  (submission) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <td style={{ padding: "16px" }}>
                                          <span
                                            className={`badge ${statusClass} px-2 py-1`}
                                          >
                                            <i
                                              className={`fas ${statusIcon} me-1`}
                                            ></i>{" "}
                                            {statusText}
                                          </span>
                                        </td>
                                        ...
                                        <td style={{ padding: "16px" }}>
                                          ...
                                          <div
                                            ...
                                            onClick={() => {
                                              ...
                                              document.getElementById(
                                                `feedback-overlay-${submission._id}`
                                              ).style.display = "none";
                                            }}
                                          ></div>
                                        </td>
                          ...
                          <div className="text-center py-5">
                            ...
                            >
                              Students haven't submitted any work for this
                              assignment yet.
                            </p>
                          </div>
                        )}
                        ...
                        {selectedClassroom.students?.length > 0 ? (
                          <div className="table-responsive">
                            <table className="table table-hover mb-0">
                              <thead style={{ backgroundColor: "#f9fafb" }}>
                                <tr>
                                  <th style={{ width: "5%", padding: "16px" }}>
                                    #
                                  </th>
                                  <th style={{ width: "10%", padding: "16px" }}>
                                    Avatar
                                  </th>
                                  <th style={{ width: "20%", padding: "16px" }}>
                                    Student
                                  </th>
                                  <th style={{ width: "20%", padding: "16px" }}>
                                    Email
                                  </th>
                                  <th style={{ width: "15%", padding: "16px" }}>
                                    Submissions
                                  </th>
                                  <th style={{ width: "15%", padding: "16px" }}>
                                    Average Grade
                              ...
                              <tbody>
                                {selectedClassroom.students.map(
                                  (student, index) => {
                                    ...
                                    const averageGrade =
                                      completedAssignments > 0
                                        ? (
                                            totalGrade / completedAssignments
                                          ).toFixed(1)
                                        : "N/A";
                                          ...
                                          >
                                            <button
                                              ...
                                              onClick={() => {
                                                ...
                                                alert(
                                                  `Student progress view for ${student.name} coming soon!`
                                                );
                                              }}
                          ...
                          <div className="text-center py-5">
                            ...
                            >
                              Add students to your classroom by inviting them
                              via email or sharing the class code.
                            </p>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  navigator.clipboard.writeText(
                                    selectedClassroom.roomCode
                                  );
                                  setSuccess("Class code copied to clipboard!");
                                }}
                              >
                                <i className="fas fa-copy me-2"></i> Copy Class
                                Code
                              </button>
                            </div>
                          </div>
          ...
          <div className="modal-content">
            <GradeSubmissionModal
              ...
              submission={{
                ...
                feedback:
                  gradeModalData?.feedback || selectedSubmission.feedback,
              }}
              onClose={() => {
                setShowGradeModal(false);
                setSelectedSubmission(null);
